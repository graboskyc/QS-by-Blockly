...
# Generated at '+d+' via version 0.5.0.1 \n\
...
Blockly.Blocks['qs_execmd'] = {
  init: function() {
    ...
    this.appendValueInput("name")
        .setCheck("String")
        .appendField("on a")
        .appendField(new Blockly.FieldDropdown([["resource", "1"], ["service", "2"]]), "type")
        .appendField("named");
    this.setInputsInline(true);
...
Blockly.Python['qs_execmd'] = function(block) {
  ...
  cmd = "";
  if (dropdown_sync == "1"){
    cmd = 'csapi.ExecuteCommand(reservation["id"],'+value_name+',"'+dropdown_type+'",'+value_cmd+',inputs[])\n\n';
  }
  else {
    cmd = 'csapi.EnqueueCommand(reservation["id"],'+value_name+',"'+dropdown_type+'",'+value_cmd+',inputs[])\n\n';
  }
  return cmd;
};
...
Blockly.Python['qs_exeall'] = function(block) {
  ...
  return 'rdet = csapi.GetReservationDetails(reservationDetails["id"])\n\
for resource in rdet.ReservationDescription.Resources:\n\
\ttry:\n\
\t\tapi.EnqueueCommand(reservation["id"], resource.Name, "Resource", "'+text_cmd+'", [], True)\n\
\texcept:\n\
\t\tcontinue\n\
\n\
for service in rdet.ReservationDescription.Services:\n\
\ttry:\n\
\t\tapi.EnqueueCommand(reservation["id"], service.Alias, "Service", "'+text_cmd+'", [], True)\n\
\texcept:\n\
\t\tcontinue\n\n';
};